...
b3kkk = Client("Channel - B3KKK",api_id=14170449,api_hash="03488b3c030fe095667e7ca22fe34954",bot_token="6404228620:AAGDLjoH4zxB7yhLPJJPuvKaRJqFUIOU4_k")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=6377763320)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await b3kkk.send_message(6377763320,f"""
		<u>«**New User**»</u>
...
@b3kkk.on_message(filters.command("تفعيل التواصل","") & filters.user(6377763320) & filters.private)
...
@b3kkk.on_message(filters.command("تعطيل التواصل","") & filters.user(6377763320) & filters.private)
...
@b3kkk.on_message(filters.command("الاحصائيات","") & filters.user(6377763320) & filters.private)
...
@b3kkk.on_message(filters.command("اذاعه للكل","") & filters.user(6377763320) & filters.private)
...
@b3kkk.on_message(filters.command("حظر عضو","") & filters.user(6377763320) & filters.private)
...
@b3kkk.on_message(filters.command("الغاء حظر عضو","") & filters.user(6377763320) & filters.private)
...
@b3kkk.on_message(filters.private & ~filters.command("start") & ~filters.user(6377763320))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await b3kkk.copy_message(chat_id=6377763320,
